...
switch ($method) {
    case 'GET': get_bookings($pdo); break;
    case 'POST': create_booking($pdo); break;
    case 'PUT': update_booking_status($pdo); break;
    default:
...
function create_booking($pdo) {
    if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'tourist') {
        http_response_code(403); echo json_encode(['message' => 'Only tourists can book tours']); return;
    }
    ...
    try {
        ...
        http_response_code(500); echo json_encode(['message' => 'Booking failed: ' . $e->getMessage()]);
    }
}
...
function update_booking_status($pdo) {
    ...
    if ($_SESSION['role'] === 'guide') {
        ...
        if (!$booking || $booking['guide_id'] != $_SESSION['user_id']) {
            http_response_code(403); echo json_encode(['message' => 'Unauthorized to update this booking']); return;
        }
    }
    try {
        ...
        http_response_code(500); echo json_encode(['message' => 'Failed to update booking']);
    }
}