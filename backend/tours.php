...
switch ($method) {
    case 'GET':
        ...
        break;
    case 'POST': create_tour($pdo); break;
    case 'PUT':
        ...
        break;
    case 'DELETE': delete_tour($pdo); break;
    default:
...
function create_tour($pdo) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO tours (guide_id, title, description, price, duration, location, image, is_approved) VALUES (?, ?, ?, ?, ?, ?, ?, 1)");
        $stmt->execute([
            $_SESSION['user_id'],
            $input['title'],
            $input['description'] ?? '',
            ...
            $input['duration'],
            $input['location'],
            (!empty($input['image'])) ? $input['image'] : null
        ]);
...
function update_tour($pdo) {
    if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'guide') {
        http_response_code(403); echo json_encode(['message' => 'Unauthorized']); return;
    }
    $input = json_decode(file_get_contents('php://input'), true);
    if (!isset($input['id'])) {
        http_response_code(400); echo json_encode(['message' => 'Tour ID required']); return;
    }
    $stmt = $pdo->prepare("SELECT guide_id FROM tours WHERE id = ?");
    $stmt->execute([$input['id']]);
    $tour = $stmt->fetch();
    if (!$tour || $tour['guide_id'] != $_SESSION['user_id']) {
        http_response_code(403); echo json_encode(['message' => 'Unauthorized to update this tour']); return;
    }
    $fields = []; $params = [];
    foreach (['title', 'description', 'price', 'duration', 'location', 'image'] as $field) {
        if (isset($input[$field])) { $fields[] = "$field = ?"; $params[] = $input[$field]; }
    }
    if (empty($fields)) { echo json_encode(['message' => 'No changes provided']); return; }
    $params[] = $input['id'];
    ...
    try {
        ...
        http_response_code(500); echo json_encode(['message' => 'Failed to update tour: ' . $e->getMessage()]);
    }
}
...
function delete_tour($pdo) {
    ...
    $id = $_GET['id'] ?? null;
    if (!$id) { http_response_code(400); echo json_encode(['message' => 'Tour ID required']); return; }
    $stmt = $pdo->prepare("SELECT guide_id FROM tours WHERE id = ?");
    $stmt->execute([$id]);
    $tour = $stmt->fetch();
    if (!$tour || $tour['guide_id'] != $_SESSION['user_id']) {
        http_response_code(403); echo json_encode(['message' => 'Unauthorized to delete this tour']); return;
    }
    try {
        ...
        http_response_code(500); echo json_encode(['message' => 'Failed to delete tour']);
    }
}
...
function approve_tour($pdo) {
    ...
    try {
        ...
        http_response_code(500); echo json_encode(['message' => 'Failed to approve tour']);
    }
}

function get_locations($pdo) {
    try {
        $stmt = $pdo->query("SELECT DISTINCT location FROM tours WHERE is_approved = 1 ORDER BY location ASC");
        $locations = $stmt->fetchAll(PDO::FETCH_COLUMN);
        echo json_encode($locations);
    } catch (PDOException $e) {
        http_response_code(500);
        echo json_encode(['message' => 'Failed to fetch locations: ' . $e->getMessage()]);
    }
}